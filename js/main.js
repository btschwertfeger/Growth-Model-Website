/**
 * Website to visualize and manipulate the Growth Model provided by Benjamin
 * Thomas Schwertfeger, Nadezhda Valerievna Sokolova, and Anja Rohner
 *
...
import { get_weight_by_temperature, get_temperature_index } from "./utils.js";

import { avg, range, quantile } from "./math.js";
...
window.computeSimple = (
  ...
  initial_weight =
    initial_weight === null
      ? parameters.initial_weight * 1000
      : parseFloat(initial_weight) * 1000; // * 1000 -> kg -> gramm

  let a_fit = data.a_fit,
    b_fit = data.b_fit,
    c_avg = data.c_avg,
    itemp = 0, //  temperature index
    dt = 1, //  time step in days
    temperature_range = range(-2, 30.5 - 0.5, 0.5); //  Temperature range in °C

  let weight_at_age = [...new Array(parameters.max_age)].map(
    // weight at age (in g) for a given temperature && initial weight in kg
    (e, i) =>
      [...new Array(a_fit.length)].map(() => (i === 0 ? initial_weight : 0)),
  );
  ...
  temperature_range.forEach(() => {
    range(1, parameters.max_age - 1, 1).forEach((age) => {
      let growth_rate =
        0.01 *
        (a_fit[itemp] * Math.pow(weight_at_age[age - 1][itemp], b_fit[itemp]) -
          c_avg);
      weight_at_age[age][itemp] =
        weight_at_age[age - 1][itemp] * (1 + dt * growth_rate);
    });
    itemp++;
  });
...
window.plot_simple = (temperature = 3.0) => {
  ...
  config.options.plugins.tooltip = {
    ...
    callbacks: {
      label: (context) => {
        return context.dataset.label !== null && context.parsed.y !== null
          ? `${context.dataset.label}: ${parseFloat(Number(context.parsed.y.toFixed(2)))}kg`
          : "";
      },
      title: (context) => {
  ...
  if (typeof window.simple_chart === "undefined")
    window.simple_chart = new Chart(ctx, config);
  else {
...
const n_generations_celtic_sea = 10,
...
window.boxplot_data = {
  ...
  Barents_Sea: {
    data: [...new Array(n_generations_barents_sea)].map(() => [
      ...new Array(0),
    ]),
  },
};
...
window.complex_charts = [
  null,
  null, // upper left, upper right,
];

window.plot_complex_precomputed = (scenario, period) => {
  window.active_scenario = scenario;
  const get_weight_by_age = (data, n_generations) => {
      let weight_by_age_vector = [...new Array(n_generations)].map(
        () => new Array(),
      );
  ...
  const get_line_datasets = (region, scenario, period) => {
    let datasets = [];
    Object.keys(region.precomputed[scenario][period]).forEach(
      (quant, qindex) => {
        const weight_by_age_vector = get_weight_by_age(
          region.precomputed[scenario][period][quant],
          region.name == "Celtic_Sea"
            ? n_generations_celtic_sea
            : n_generations_barents_sea,
        );
        datasets.push({
          type: "line",
          ...
          ].map((e, i) => avg(weight_by_age_vector[i])),
          borderColor: quant.includes("50")
            ? "black"
            : quant.includes("25")
              ? "orange"
              : "red",
  ...
  let general_config = JSON.parse(
    JSON.stringify(window.default_chart_config_simple),
  );
  general_config.data.datasets = [];
  general_config.options.scales.x.title.text = "Age, years";
  general_config.options.scales.x.min = 1;
  general_config.options.scales.x.max = Math.max(
    n_generations_celtic_sea,
    n_generations_barents_sea,
  );
  ...
  if (scenario == "SODA") {
    for (let regionidx = 0; regionidx < window.regions.length; regionidx++) {
      ...
      if (window.complex_charts[regionidx]) {
        ...
        config.data.datasets.push(
          get_soda_boxplot(window.boxplot_data[region.name].data),
        );
          ...
          callbacks: {
            label: (context) => {
              ...
              if (
                ...
                context.dataset.label === "Observation"
              ) {
                if (context.parsed.min !== null)
                  label += `min: ${Number(context.parsed.min).toFixed(2)}kg; `;
                if (context.parsed.q1 !== null)
                  label += `25% Quantile: ${Number(context.parsed.q1).toFixed(2)}kg; `;
                if (context.parsed.median !== null)
                  label += `median: ${Number(context.parsed.median).toFixed(2)}kg; `;
                if (context.parsed.mean !== null)
                  label += `mean: ${Number(context.parsed.mean).toFixed(2)}kg; `;
                if (context.parsed.q3 !== null)
                  label += `75% Quantile: ${Number(context.parsed.q3).toFixed(2)}kg; `;
                if (context.parsed.max !== null)
                  label += `max: ${Number(context.parsed.max).toFixed(2)}kg`;
              } else
      ...
      get_line_datasets(window.regions[regionidx], scenario, period).forEach(
        (dataset) => config.data.datasets.push(dataset),
      );
      ...
      if (!window.complex_charts[regionidx])
        window.complex_charts[regionidx] = new Chart(
          $(`#complex_plot_canvas${regionidx}`),
          config,
        );
      else config.update();
    }
    ...
    for (let regionidx = 0; regionidx < window.regions.length; regionidx++) {
      let config = window.complex_charts[regionidx],
        region = window.regions[regionidx];

      config.data.datasets.splice(1);
      // * LINEPLOT
      get_line_datasets(region, scenario, period).forEach((dataset) =>
...
/*
 ...
 /_/   \_\__,_|/ |\__,_|___/\__\__,_|_.__/|_|\___|
                |__/ Experiment
*/
window.default_constants = {
  // # Values from Eg. 2 Butzin and Poertner, 2016)
  A_R: 8.66,
  B_R: 0.3055,
  THETA_A: 18145,
  THETA_B: 4258,
...
let equation2 = (input_temp, constants) => {
    if (input_temp < -988) return -999;
      ...
      const a_numerator =
          constants.A_R *
          Math.exp(
            constants.THETA_A / constants.T_R - constants.THETA_A / T_Kelvin,
          ),
        a_dominator =
          1 +
          Math.exp(
            constants.THETA_H / constants.T_H - constants.THETA_H / T_Kelvin,
          );
  ...
  equation3 = (input_temp, constants) => {
    ...
    if (input_temp < -998) return -999;
      ...
      return (
        constants.B_R *
        Math.exp(
          constants.THETA_B / constants.T_R - constants.THETA_B / T_Kelvin,
        )
      );
    }
  },
  get_adj_year = (year, data) => {
    const index =
      12 * (year - window.default_input_complex_adjustable.initial_year); // x12 months
...
window.computeComplexAdjustable = (constants = null) => {
  ...
  while (last_year < settings.years.at(-1)) {
    ...
    for (let year = initial_year; year < last_year; year++) {
      ...
      let a = [...new Array(settings.n_depths)].map(() => 1),
        ...
        for (let day = 0; day < n_days_per_month; day++) {
          for (let level = 0; level < settings.n_depths; level++) {
            if (tmp[month][level] < -998) {
              ...
              growth_rates[level] =
                0.01 *
                (a[level] * Math.pow(weight[level], b[level]) -
                  constants.C_AVG);
              growth_rates[level] =
                growth_rates[level] < 0 ? 0 : growth_rates[level];
              weight[level] =
                weight[level] < -998
                  ? -999
                  : weight[level] * (1.0 + dt * growth_rates[level]);
            }
          }
        }
      }
      ...
      const content = {
        ...
        weight: weight.map((e) => (e < -998 ? -999 : e * 0.001)),
        weight_max: Math.max(
          ...weight.map((e) => (e < -998 ? -999 : e * 0.001)),
        ),
      };
...
window.plot_complex_adjustable = (data = null) => {
  ...
  const settings = window.default_input_complex_adjustable,
    ctx = $(`#complex_plot_canvas_adjustable`);

  // ? create array that contains the weights of all depths per age
  let weight_by_age_vector = [...new Array(settings.generation)].map(
    () => new Array(),
  );
  for (
    let year = settings.initial_year;
    year < settings.last_year + 1;
    year++
  ) {
  ...
  for (let age = 0; age < settings.generation; age++) {
    const quantiles = {
      q_25: quantile(weight_by_age_vector[age], 0.25),
      q_50: quantile(weight_by_age_vector[age], 0.5),
      q_75: quantile(weight_by_age_vector[age], 0.75),
    };
    if (typeof weight_by_age_quantile[`${age}`] === "undefined")
      weight_by_age_quantile[`${age}`] = {
        q_25: [quantiles.q_25],
        q_50: [quantiles.q_50],
        q_75: [quantiles.q_75],
      };
  ...
  let plottable_datasets = [
    {
      type: "line",
      label: "25% Quantile",
      data: [...new Array(years)].map((e, i) =>
        avg(weight_by_age_quantile[i].q_25),
      ),
      fill: false,
      borderColor: "orange",
      pointRadius: 0,
    ...
    },
    {
      ...
      label: "50% Quantile",
      data: [...new Array(years)].map((e, i) =>
        avg(weight_by_age_quantile[i].q_50),
      ),
      fill: false,
      borderColor: "black",
      pointRadius: 0,
    },
    {
      ...
      label: "75% Quantile",
      data: [...new Array(years)].map((e, i) =>
        avg(weight_by_age_quantile[i].q_75),
      ),
      fill: false,
      borderColor: "red",
      pointRadius: 0,
  ...
  if (kind == "Simple")
    ...
    const gather_lines = (region_name, scenario, period, quant, lines) => {
      ...
      for (let i = 0; i < lines.length; i++)
        lines[i].split(",").forEach((e, i) => {
          if (i !== 0) {
            if (region_name == "Celtic_Sea" && i > 10)
              data.push(parseFloat(0.0)); // Celtic_Sea only has 10 generations
      ...
      for (let i = 0; i < window.regions.length; i++)
        if (window.regions[i].name == region_name) {
          if (scenario == "SODA")
            window.regions[i].precomputed[`${scenario}`][period][quant] = data;
          else
            window.regions[i].precomputed[`FESOM_${scenario}`][period][quant] =
              data;
        }
    };

    const gather_quant = (full_descr, region_name, scenario, period, lines) => {
      if (full_descr.includes("quant25"))
        gather_lines(region_name, scenario, period, "quant25", lines);
      else if (full_descr.includes("quant50"))
        gather_lines(region_name, scenario, period, "quant50", lines);
      else if (full_descr.includes("quant75"))
        gather_lines(region_name, scenario, period, "quant75", lines);
    };

    const gather_period = (full_descr, region_name, scenario, lines) => {
      if (full_descr.includes("1981_2000"))
        gather_quant(full_descr, region_name, scenario, "1981_2000", lines);
      else if (full_descr.includes("2081_2100"))
        gather_quant(full_descr, region_name, scenario, "2081_2100", lines);
      else if (full_descr.includes("2181_2200"))
        gather_quant(full_descr, region_name, scenario, "2181_2200", lines);
    };

    const gather_scenario = (full_descr, region_name, lines) => {
      if (full_descr.includes("rcp45"))
        gather_period(full_descr, region_name, "rcp45", lines);
      else if (full_descr.includes("rcp85"))
        gather_period(full_descr, region_name, "rcp85", lines);
      else if (full_descr.includes("SODA"))
        gather_period(full_descr, region_name, "SODA", lines);
    };
    ...
    for (let line = 0; line < allTextLines.length - 1; line++) {
      ...
      if (description.includes("Celtic_Sea")) {
        gather_scenario(
          description,
          "Celtic_Sea",
          allTextLines.slice(line + 1, line + 21),
        );
        line += 20;
    ...
    for (let line = 0; line < allTextLines.length - 1; line++) {
      ...
      const gather_scenario = (scenario_name, data) => {
        data.split(",").forEach((e, i) => {
          if (i != 0 && i < window.boxplot_data[scenario_name].data.length + 1)
            window.boxplot_data[scenario_name].data[i - 1].push(parseFloat(e));
        });
      };
      if (
        description.includes("Celtic_Sea") ||
        (current_scenario == "Celtic_Sea" &&
          !description.includes("Barents_Sea"))
      ) {
        current_scenario = "Celtic_Sea";
        if (!description.includes("Celtic_Sea"))
          gather_scenario("Celtic_Sea", allTextLines[line]);
      } else if (
        description.includes("Barents_Sea") ||
        ...
        current_scenario = "Barents_Sea";
        if (!description.includes("Barents_Sea"))
          gather_scenario("Barents_Sea", allTextLines[line]);
      }
    }
    ...
    ["Celtic_Sea", "Barents_Sea"].forEach((region_name) => {
      window.boxplot_data[region_name].data.forEach((e, i) => {
        window.boxplot_data[region_name].data[i] = window.boxplot_data[
          region_name
        ].data[i].filter((value) => !Number.isNaN(value));
      });
    });
...
$(document).ready(() => {
  ...
  /**
     ...
     * ? Python code to get the .csv file in the correct form:
        ...
        # this can be found in /data/validate_adjustable_1loc.ipynb
     */
  $.ajax({
    type: "GET",
    ...
    dataType: "text",
    success: (data) => {
      window.processed_data_complex_adjustable = processData(
        data,
        "complex-one-region",
      );
